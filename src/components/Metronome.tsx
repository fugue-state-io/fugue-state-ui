  ...
  const scheduleRepeat = () => {
    ...
    const loop = new Tone.Loop((time: number) => {
      ...
      const nextStep = Math.round(
        time / Tone.Time(props.subdivisions * 4 + "n").toSeconds()
      );
      if (synth && enabled) {
    ...
    for (let i = 0; i < timeSignature; i++) {
      content.push(
        <span
          ...
          key={i.toString()}
          className={
            "relative px-2 mx-1 rounded-md shadow-lg shadow-lg " +
            (step % timeSignature == i ? "bg-pink-400" : "bg-pink-700")
          }
        ></span>
      );
    }
    ...
    <div>
      <div className="bg-gray-900 text-center py-4">
        <div className="max-w-md grid grid-cols-5 text-center mx-auto relative my-2">
          <div className="flow-root grid-cols-1 px-1 leading-none align-middle">
            ...
            <input
              name="bpm"
              id="bpm"
              onChange={(event) => props.setMpm(parseInt(event?.target.value))}
              type="number"
              value={props.mpm}
              pattern="\d+"
          ...
          <div className="flow-root grid-cols-1 px-1 leading-none align-middle">
            ...
            <input
              ...
              id="timeSignature"
              onChange={(event) =>
                props.setSubdivisions(parseInt(event?.target.value))
              }
              type="number"
          ...
          <div className="flow-root grid-cols-1 px-1 leading-none align-middle">
            ...
            <input
              ...
              id="phaseOffset"
              onChange={(event) =>
                props.setDelay(parseInt(event?.target.value))
              }
              type="number"
              value={props.delay}
              pattern="\d+"
        ...
        <div
          className={
            enabled
              ? "items-center rounded-md py-12"
              : "items-center rounded-md py-12 hidden"
          }
        >
